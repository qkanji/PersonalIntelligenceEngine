...
export default function Chat() {
  ...
  return (
    <div className="flex flex-col h-full w-full relative pb-4 md:pb-6">
      {/* Top Header */}
      <header className="flex justify-between items-center py-3 px-4 md:px-6">
      ...
      {/* Main Chat Area */}
      <div className="flex-1 overflow-y-auto mt-4 px-4 md:px-6 no-scrollbar">
        {!hasMessages ? (
          <div className="flex flex-col items-center justify-center h-full min-h-[50vh] pb-32">
            <div className="w-full max-w-2xl px-4 lg:px-0 flex flex-col gap-6 text-center">
              <h2 className="text-[40px] leading-tight font-medium text-slate-700 font-sans tracking-tight mb-8">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-500 font-semibold flex items-center justify-center gap-3">
          ...
          <div className="w-full max-w-3xl mx-auto pb-32">
            {messages.map((message, index) => (
              <MessageBubble
                key={message.id}
                message={message}
                // In ai@3.4, system/stream data might be in message.annotations or top-level data array.
                // We'll pass the specific data index.
                streamData={data}
              />
            ))}
            {isLoading && (
      ...
      {/* Bottom Input Area */}
      <div
        className={`w-full max-w-3xl mx-auto transition-all duration-500 ease-in-out ${hasMessages ? "absolute bottom-0 md:bottom-2 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl border border-slate-100 p-2 md:p-3 rounded-[2rem] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)] w-[calc(100%-2rem)]" : "absolute top-1/2 left-1/2 -translate-x-1/2 mt-16 px-4 w-full"}`}
      >
        {!hasMessages && (
          <div className="w-full max-w-2xl mx-auto mb-6">
            ...
            <div className="flex gap-3 justify-center mt-6 overflow-x-auto pb-2 no-scrollbar pl-2 md:pl-0">
              <Button
                variant="outline"
                className="rounded-full bg-white border-slate-200 text-slate-600 gap-2 h-11 px-5 shadow-sm font-normal"
              >
              ...
              <Button
                variant="outline"
                className="rounded-full bg-white border-slate-200 text-slate-600 gap-2 h-11 px-5 shadow-sm font-normal hidden sm:flex"
              >
        ...
        {hasMessages && (
          ...
          >
            <input
              className="flex-1 bg-transparent border-0 outline-none text-slate-800 text-[15px] placeholder:text-slate-400 py-3 pr-2 pl-2"
